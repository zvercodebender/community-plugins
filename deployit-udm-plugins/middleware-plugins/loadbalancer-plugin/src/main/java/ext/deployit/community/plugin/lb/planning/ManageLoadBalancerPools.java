...
public class ManageLoadBalancerPools extends ContainerContributor<GenericContainer> {
    ...
    public void manageContainersInPool(Deltas deltas, DeploymentPlanningContext ctx) {
        ...
        Set<Container> loadBalancers = getMembersOfType(targetEnvironment, LOADBALANCER_TYPE);
        ...
        Map<GenericContainer, List<Container>> containerLoadBalancers = getContainerLoadBalancers(loadBalancers);
        
        /* 
         * container -> first/last deployment groups of the fronted servers
         * 
         * Deployment groups are identified by the 'deploymentGroup' property
         * which is sorted numerically. So if the deployment groups are
         * 
         * - 3
         * - 6
         * - 2
         * 
         * the first group is 2 and the last group is 6. Containers without 
         * tags are in the special Integer.MAX_VALUE group that is processed 
         * last.
         */
        Map<GenericContainer, LoadBalancingBounds> containerBalancingBounds = 
            getLoadBalancingBounds(containerLoadBalancers.keySet());
        
        /*
         * The deployment group that the contributor is currently processing. This
         * can be found by looking at the deltas currently being processed - the
         * group-based orchestrator will only include a delta in the current set if
         * its container is in the current deployment group. 
         * 
         * So by looking at the deployment groups of the containers being processed 
         * we can find the current group (if the containers have no group, we are 
         * in the 'unallocated' group).
         */
        ...
        for (Entry<GenericContainer, LoadBalancingBounds> balancingBounds : containerBalancingBounds.entrySet()) {
            GenericContainer container = balancingBounds.getKey();
    ...
    private Integer getCurrentDeploymentGroup(Set<GenericContainer> containers) {
    ...
    private static void addRemoveFromPoolSteps(DeploymentPlanningContext ctx,
            GenericContainer container, List<Container> loadBalancers) {
    ...
    private static void addReturnToPoolSteps(DeploymentPlanningContext ctx, 
            GenericContainer container, List<Container> loadBalancers) {
        for (Container loadBalancer : loadBalancers) {
